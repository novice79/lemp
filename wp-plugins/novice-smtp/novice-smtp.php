<?php
/*
  Plugin Name: Novice SMTP
  Version: 1.0.0
  Author: novice
  Description: Send email via SMTP from your WordPress
  Text Domain: novice-smtp
...
class NoviceSMTP {
	...
	require_once('novice-smtp-utils.php');
	...
	if ( is_admin() && ! (defined( 'DOING_AJAX' ) && DOING_AJAX) ) {
	    require_once('novice-smtp-admin-menu.php');
...
//PHPMailer 5.2.10 introduced this option. However, this might cause issues if the server is advertising TLS with an invalid certificate.
	...
	if ( $is_import_settings ) {
	    $err_msg = __( 'Error occurred during settings import', 'novice-smtp' );
	...
	if ( ! $this->credentials_configured() ) {
	    ...
	    ?>
	    <div class="error">
	        <p><?php printf( __( 'Please configure your SMTP credentials in the <a href="%s">settings menu</a> in order to send email using Easy WP SMTP plugin.', 'novice-smtp' ), esc_url( $settings_url ) ); ?></p>
	    </div>
	    <?php
	}
	...
	if ( $settings_import_notice ) {
	    ...
	    ?>
	    <div class="updated">
	        <p><?php _e( 'Settings have been imported successfully.', 'novice-smtp' ); ?></p>
	    </div>
	    <?php
	}
    }
	...
	if ( $file == plugin_basename( $this->plugin_file ) ) {
	    $settings_link = '<a href="options-general.php?page=swpsmtp_settings">' . __( 'Settings', 'novice-smtp' ) . '</a>';
	...
	if ( $file == plugin_basename( $this->plugin_file ) ) {
	    $links[] = '<a href="options-general.php?page=swpsmtp_settings">' . __( 'Settings', 'novice-smtp' ) . '</a>';
	}
	return $links;
    }

    function plugins_loaded_handler() {
	load_plugin_textdomain( 'novice-smtp', false, dirname( plugin_basename( $this->plugin_file ) ) . '/languages/' );
    }
	...
	$err_msg = __( 'Please refresh the page and try again.', 'novice-smtp' );
